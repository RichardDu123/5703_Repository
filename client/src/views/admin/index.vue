<template>
  <div class="adminContainer">
    ...
    <div class="d2">Add Electricity to a user account</div>
    ...
    <div class="d4">
      <div class="how_many">Electricity Unit + '(KW)'</div>
...
import { getUsernameByAddress, addAvailableElecUnits } from '../../api/mainSys'
...
function handleSubmit() {
  ...
  addAvailableElecUnits(contract, address, address1, amount).then((value) => {
    if (a_put.value.length >= 15 && e_put.value.length >= 0) {
      ...
      ElMessage({
        message: 'You have add ' + ad_data.unit + ' in ' + ad_data.address,
        type: 'success',
      })
    } else {
      ElMessage.error('Address is not match')
    }
  })
}
